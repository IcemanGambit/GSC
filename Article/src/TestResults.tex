...
We see that the simulation resembles the real-world results, however, the simulated vehicles seem to hold longer at the traffic lights. 
The average waiting time is not that different (see Table~\ref{table.valMetric}), indicating that this is just a small difference that accumulates over time. %TODO: This is not good.
The main reason for the difference is because actual the phases of traffic lights are estimated. 
The actual phases was not avaiable, and hence, we had to estimate them based on the GPS trajecotories and local knowledge of the are.
...
Therefore, it will be interessting to know the effeciency of \tech when only 10 \% of the vehicles use it. 
By looking at different penetration rates, it will be possible to understand how scalable the system is.
If the effeciency is independend on the penetration rate, and the first vehicle using \tech will benefit as much as the last vehicle, then the system will be highly scalable.
...
It is very important that neither the fuel consumption or the travel time of vehicles not using \tech increases, because \tech must be fair to all vehicles. %TODO: reformulate this!
...
In order to test the effect of the system with different congestion levels we repeated the simulation while changing the rate with which vehicles are spwaning (see Section~\ref{sec:UseCase}). 
...
The results at congestion levels above 1.6 vehicles per second are therefore not reliable not included in the results.

When noone use \tech, we see that the fuel consumption and travel time increases rapidly at 1 vehicle per second, indicating that the network becomes too loaded at this congestion level. 
...
It therefore looks like the network can handle more vehicles when they use \tech.
